...
initDb conn = do
  ...
  execute_ conn "CREATE TABLE IF NOT EXISTS person_companies (person_id INTEGER NOT NULL REFERENCES people(id) ON DELETE CASCADE, company_id INTEGER NOT NULL REFERENCES companies(id) ON DELETE CASCADE, PRIMARY KEY (person_id, company_id))"
  execute_ conn "CREATE TABLE IF NOT EXISTS event_companies (event_id INTEGER NOT NULL REFERENCES events(id) ON DELETE CASCADE, company_id INTEGER NOT NULL REFERENCES companies(id) ON DELETE CASCADE, PRIMARY KEY (event_id, company_id))"
